...
class Match:
    ...
    def save_match(self):
        serialized_match = self.serialized_match()
        db = TinyDB('db.json')
    ...
    def maj_results(self):
        ...
        table.update({"results_player_1": str(self.results_player_1)},
                     where("id_match") == self.id_match.hex)
        table.update({"results_player_2": str(self.results_player_2)},
                     where("id_match") == self.id_match.hex)
    ...
    @staticmethod
    def historic_match(round_instance,
                       index_player_1,
                       index_player_2,
                       players):
        for i in range(len(round_instance)):
            ...
            for k in range(len(tour)):
                match = tour[k]
                if (match[0][0] == players[index_player_1]
                    and match[1][0] == players[index_player_2]) \
                        or (match[0][0] == players[index_player_2]
                            and match[1][0] == players[index_player_1]):
                    return True
        return False
